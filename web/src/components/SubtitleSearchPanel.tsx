      ...
      <div className="w-full max-w-2xl max-h-[80vh] flex flex-col rounded-2xl" style={{
        ...
        <div className="flex items-center justify-between p-6 pb-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-display text-lg font-semibold text-white">在线字幕搜索</h3>
              {title && <p className="text-xs text-surface-400">{title} {year ? `(${year})` : ''}</p>}
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 p-6">
          <select value={language} onChange={e => setLanguage(e.target.value)}
            ...
            <option value="zh-cn,en">中文 + 英文</option>
        ...
        <div className="flex-1 overflow-y-auto p-6 pt-3 space-y-2">
          {results.map(sub => (
            <div key={sub.id} className="flex items-center gap-3 rounded-xl bg-surface-800/50 p-3 hover:bg-surface-800/80 transition-colors">
              <div className="flex-1 min-w-0">
                <p className="text-sm text-white truncate">{sub.file_name}</p>
                <div className="flex items-center gap-3 mt-1 text-xs text-surface-400">
                  ...
                  <span className="uppercase text-surface-500">{sub.format}</span>
                </div>
              </div>
              <button
                onClick={() => handleDownload(sub)}
                disabled={downloading === sub.id}
                className="btn-ghost shrink-0 rounded-lg px-3 py-1.5 text-xs flex items-center gap-1"
              >
                {downloading === sub.id ? (
          ...
          {results.length === 0 && !searching && (
            <div className="text-center py-12 text-surface-500">
              ...
              <p className="text-xs mt-1">数据源: OpenSubtitles</p>
            </div>
          )}
        </div>
      </div>
