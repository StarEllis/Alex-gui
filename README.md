# <img src="./build/appicon.png" alt="ALEX Logo" width="40" valign="middle" /> ALEX Desktop

一个基于 `Go + Wails + React` 的本地媒体库桌面应用，重点解决本地影视资源的整理、扫描、浏览、播放和基础远程访问问题。

ALEX 不是 Electron 套壳网页，而是用 Go 做桌面后端、React 做界面、SQLite 做本地数据存储的轻量级桌面程序，适合在 Windows 桌面环境中管理自己的本地媒体库。

![ALEX Desktop Preview](./assets/readme-preview.png)
...
- `build/bin/alex-desktop.exe`

推荐安装步骤：

1. 新建一个可写目录，例如 `D:\Apps\ALEX` 或 `C:\Users\<你的用户名>\Apps\ALEX`
2. 将 `alex-desktop.exe` 放进去
3. 双击运行
...
```bash
git clone https://github.com/StarEllis/Alex-gui.git alex-desktop
cd alex-desktop
```
...
如果你想让 Infuse 或其他客户端访问 ALEX 的媒体库：
...
├─ alex.db                     # 本地 SQLite 数据库（运行后生成/更新）
