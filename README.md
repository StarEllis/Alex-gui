...
### 📺 智能播放
- **直接播放** — MP4 / WebM / M4V 等浏览器兼容格式零延迟直接播放，支持 Range 请求
...
- **转码任务监控** — 状态、进度、速度、取消
- **用户管理** — 列表、删除、权限设置
- **API Key 在线配置** — TMDb / Bangumi / TheTVDB / Fanart.tv 配置 / 修改 / 清除 / 掩码展示
- **系统全局设置** — 在线修改系统参数
...
<details>
<summary><b>流媒体 / 字幕 / 搜索 / 推荐</b></summary>
...
<details>
<summary><b>用户 / 收藏 / 历史 / 播放列表 / 书签 / 评论 / 统计</b></summary>
...
<details>
<summary><b>投屏 / WebSocket</b></summary>
...
| GET | `/api/admin/stats/:userId` | 用户统计 |

</details>

<details>
<summary><b>转码 / 配置 / 定时任务 / 批量操作</b></summary>
...
<details>
<summary><b>元数据管理 / 图片管理</b></summary>
...
| GET/PUT/DELETE | `/api/admin/permissions/:userId` | 用户权限管理 |
...
<details>
<summary><b>Bangumi / 刮削 / 文件管理 / AI / 备份</b></summary>
...
└──────┬──────┘     └─────────────┘     └───────┬───────┘
       ...
       │
       │  ┌──────────────────┐     ┌────────────────┐
       ├──│  PlaybackStats   │     │ UserPermission │
       │  │ media_id/date    │     │ allowed_libs   │
       │  │ watch_minutes    │     │ max_rating     │
       │  └──────────────────┘     │ daily_limit    │
       │                           └────────────────┘
       │
       └──────────────────────
...
│   │   ├── stream.go               #   直接播放 / HLS 流 / 海报
│   │   ├── subtitle.go             #   字幕轨道 / 提取 / 外挂
│   │   ├── metadata.go             #   元数据刮削
│   │   ├── admin.go                #   系统管理入口
│   │   ├── admin_media.go          #   管理员媒体操作
│   │   ├── admin_metadata.go       #   管理员元数据搜索
...
│   │   ├── live.go                 #   实时直播
│   │   ├── pulse.go                #   Pulse 动态
│   │   ├── emby_compat.go          #   Emby 兼容 API
│   │   ├── v2_handlers.go          #   V2 扩展处理器
│   │   ├── v4_features.go          #   V4 功能（分享/标签/匹配规则）
│   │   └── ws.go                   #   WebSocket 入口
│   ├── middleware/
│   │   ├── middleware.go            #   JWT 认证 / CORS / 管理员权限
│   │   └── security.go             #   安全头中间件
│   ├── model/                       # GORM 数据模型（30+ 张表）
│   │   ├── model.go                #   核心模型定义
│   │   ├── scrape_task.go          #   刮削任务模型
│   │   └── v3_models.go            #   V3 扩展模型
...
│   │   ├── repo_cache.go           #   缓存数据访问
│   │   ├── repo_pulse.go           #   Pulse 数据访问
│   │   ├── repo_v4.go              #   V4 功能数据访问
...
│       ├── media.go                #   媒体查询服务
│       ├── series.go               #   剧集合集服务
│       ├── scanner.go              #   文件扫描 + FFprobe + 剧集识别
│       ├── stream.go               #   流媒体服务
│       ├── transcode.go            #   FFmpeg 转码（硬件加速）
│       ├── metadata.go             #   TMDb 元数据集成
...
│       ├── scheduler.go            #   定时任务调度
│       ├── monitor.go              #   系统监控服务
│       ├── file_watcher.go         #   文件监听服务
│       ├── file_manager.go         #   文件管理服务
│       ├── scrape_manager.go       #   刮削管理服务
│       ├── scrape_delay.go         #   刮削延迟控制
│       ├── backup.go               #   数据备份与恢复
...
│       ├── pulse.go                #   Pulse 动态服务
│       ├── emby_compat.go          #   Emby 兼容服务
│       ├── v4_features.go          #   V4 功能服务
│       ├── errors.go               #   错误定义
│       └── *_test.go               #   测试文件（8 个）
├── web/                             # React 前端
│   ├── src/
│   │   ├── App.tsx                 #   路由配置（18 个页面路由）
│   │   ├── api/
...
│   │   │   ├── CreateShareDialog.tsx#     创建分享对话框
│   │   │   └── ...                  #     更多组件
│   │   ├── components/admin/        #   管理后台子组件（11 个文件）
│   │   │   ├── DashboardTab.tsx     #     仪表板
│   │   │   ├── ShareManager.tsx     #     分享管理
│   │   │   ├── TagManager.tsx       #     标签管理
│   │   │   ├── MatchRulesTab.tsx    #     匹配规则
│   │   │   ├── LiveSourcesTab.tsx   #     直播源管理
│   │   │   └── ...                  #     更多管理组件
│   │   ├── components/media/        #   媒体详情子组件
│   │   ├── components/file-manager/ #   文件管理子组件（9 个文件）
│   │   ├── hooks/
...
- 组件按功能分目录：`components/`、`components/media/`、`components/admin/`、`components/file-manager/`

### 调试技巧

1. **开启 Debug 模式**：设置 `NOWEN_APP_DEBUG=true`，启用详细日志
2. **WebSocket 调试**：浏览器开发者工具 → Network → WS 标签页查看实时消息
3. **数据库调试**：使用 SQLite 客户端直接查看 `data/nowen.db`
4. **转码调试**：查看 `cache/transcode/` 目录下的转码输出
5. **API 调试**：Vite 开发服务器自动代理 `/api` 到后端
...
| v0.6 | V2 功能（多用户配置/离线下载/ABR/插件/音乐/图片/联邦） | ✅ 已完成 |
| v0.7 | V3 功能（AI 场景/家庭社交/直播/云同步） | ✅ 已完成 |
| v0.8 | 入口整合优化 + 多语言国际化 | ✅ 已完成 |
| v0.9 | V4 功能（分享管理/标签管理/匹配规则/Pulse 动态/Emby 兼容） | ✅ 已完成 |
...
- [ ] **移动端原生应用** — React Native / Flutter 客户端
- [ ] **智能家居集成** — HomeAssistant / Apple HomeKit 联动
- [ ] **AI 字幕翻译** — LLM 实时字幕翻译
- [ ] **AI 内容审核** — 自动识别不适宜内容
- [ ] **分布式转码** — 多节点协同转码
- [ ] **WebDAV 支持** — 兼容更多客户端
- [ ] **PostgreSQL 支持** — 可选大规模部署数据库
