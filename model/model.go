...
type Library struct {
	...
	// 媒体库元数据策略：local_only / local_preferred / online_preferred
	// local_only: 仅读取本地 NFO 和图片，不自动触发在线刮削
	// local_preferred: 优先本地 NFO，缺失时走在线刮削
	// online_preferred: 默认行为，保持现有逻辑
	MetadataMode string `json:"metadata_mode" gorm:"type:text;default:online_preferred"`
	// 实时文件监控（媒体库级别设置）
